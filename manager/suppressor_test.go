...
func (sc *testSuppressorScenario) test(i int, t *testing.T) {
	...
	// Add suppressions
	// Get each element by ID after adding it
	// Check that inhibited events are inhibited
	// Check that uninhibited events are uninhibited
	// Get summary, check that length correct
	// For each event, delete it, check that it's no longer found
